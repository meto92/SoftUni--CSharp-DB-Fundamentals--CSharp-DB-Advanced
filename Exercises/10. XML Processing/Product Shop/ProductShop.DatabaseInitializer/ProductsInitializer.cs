...
namespace ProductShop.DatabaseInitializer
{
    ...
    {
        private const string ProductsXmlPath = @"Resources\products.xml";

        private static ProductDto[] ReadProducts()
        {
            XmlRootAttribute xmlRoot = new XmlRootAttribute("products");

            XmlSerializer serializer = new XmlSerializer(typeof(ProductDto[]), xmlRoot);

            ProductDto[] deserializedProducts =
                (ProductDto[]) serializer
                    .Deserialize(new StringReader(File.ReadAllText(ProductsXmlPath)));
